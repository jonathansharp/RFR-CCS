...
headers = {'Dist' 'SSS' 'SSH' 'SST' 'CHL' 'MLD' 'Wind' 'aCO2' 'Year' 'Month1' 'Month2'};
% Standard case:
...
for k=1:numel(moornames)
    ...
    for m=1:max(SOCATv2021_grid.month_since_1998)
        ...
        idxlon = [idxlon;find(abs(SOCATv2021_grid.longitude(:,1,1)-mean(MOORING.(moornames{k}).lon,'omitnan')) == ...
            min(min(abs(SOCATv2021_grid.longitude(:,1,1)-mean(MOORING.(moornames{k}).lon,'omitnan')))))];
        idxlat = [idxlat;find(abs(SOCATv2021_grid.latitude(1,:,1)-mean(MOORING.(moornames{k}).lat,'omitnan')) == ...
            min(min(abs(SOCATv2021_grid.latitude(1,:,1)-mean(MOORING.(moornames{k}).lat,'omitnan')))))];
        idxmon = [idxmon;m];
    end
end
X_moor = [];
...
X_moor = [X_moor;...
     SOCATv2021_grid.distance_from_shore(idxlon(n),idxlat(n),idxmon(n)) ...
     SOCATv2021_grid.SSS(idxlon(n),idxlat(n),idxmon(n)) ...
     SOCATv2021_grid.SSH(idxlon(n),idxlat(n),idxmon(n)) ...
     SOCATv2021_grid.SST(idxlon(n),idxlat(n),idxmon(n)) ...
     log10(SOCATv2021_grid.CHL(idxlon(n),idxlat(n),idxmon(n))) ...
     log10(SOCATv2021_grid.MLD(idxlon(n),idxlat(n),idxmon(n))) ...
     SOCATv2021_grid.wind_speed(idxlon(n),idxlat(n),idxmon(n)).^2 ...
     SOCATv2021_grid.pCO2_atm(idxlon(n),idxlat(n),idxmon(n)) ...
     SOCATv2021_grid.year_since_1997(idxlon(n),idxlat(n),idxmon(n)) ...
     SOCATv2021_grid.month_sin(idxlon(n),idxlat(n),idxmon(n)) ...
     SOCATv2021_grid.month_cos(idxlon(n),idxlat(n),idxmon(n))];
end
...
% Plot measured vs. predicted pCO2 values (moorings)
figure; hold on;
set(gca,'fontsize',16);
colormap(cmocean('thermal'));
